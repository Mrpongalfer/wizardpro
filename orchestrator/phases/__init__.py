...
class Phase(ABC):
    ...
    def _call_llm(
        ...
        is_sub_injection: bool = False,
    ) -> Optional["LLMOutput"]:
        """Helper method to assemble prompt, call LLM, and handle basic logging/history."""
        phase_for_logging = context.current_phase
        logger.debug(
            f"Preparing LLM call for prompt key '{prompt_key}' within phase '{phase_for_logging}'"
        )

        sub_key = prompt_key if is_sub_injection else None
        main_key = (
            self.phase_name_key if not is_sub_injection else context.current_phase
        )
        ...
        try:
            assembled_prompt = self.prompt_manager.assemble_prompt(
                phase_name=main_key,
                context=context,
                sub_injection_key=sub_key,
                sub_injection_data=template_data,
            )
            if assembled_prompt.startswith("Error:") or assembled_prompt.startswith(
                "--- ERROR"
            ):
                ...
                context.add_log_entry(
                    phase_for_logging,
                    "ERROR",
                    f"Prompt assembly failed for key '{prompt_key}'",
                    assembled_prompt,
                )
            ...
            if not target_model:
                ...
                if not target_model:
                    ...
                    target_model = (
                        "gpt-3.5-turbo"  # Using 3.5-turbo as a more accessible default
                    )
                    logger.warning(
                        f"No specific model requested or configured, using default fallback: {target_model}"
                    )
            ...
            llm_input_data = LLMInput(
                prompt=assembled_prompt, model_identifier=target_model
            )
            ...
            context.add_llm_call(
                llm_output
            )  # Add result to history regardless of error
            ...
            if llm_output.error:
                ...
                context.add_log_entry(
                    phase_for_logging,
                    "ERROR",
                    f"LLM call failed for model '{target_model}' on prompt key '{prompt_key}'",
                    llm_output.error,
                )
            ...
            logger.critical(
                f"Unexpected critical error during LLM call setup/execution in phase '{phase_for_logging}': {e}",
                exc_info=True,
            )
            context.add_log_entry(
                phase_for_logging,
                "CRITICAL",
                f"Unexpected error in _call_llm for prompt key '{prompt_key}'",
                str(e),
            )
            context.update_status("Error")
            return None

    def _update_context_and_proceed(
    ...
    def _handle_error_and_halt(
        ...
        logger.error(
            f"Phase '{self.phase_name_key}' encountered an error: {error_message}"
