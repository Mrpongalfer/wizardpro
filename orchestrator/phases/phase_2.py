...
class Phase2(Phase):
    ...
    def run(self, context: ProjectContext) -> ProjectContext:
        ...
        if not context.refined_requirements or not isinstance(
            context.refined_requirements, dict
        ):
            return self._handle_error_and_halt(
                context, "Refined requirements missing or invalid for Phase 2."
            )
        if context.refined_requirements.get("parsing_status", "").startswith("Failed"):
            logger.warning(
                "Phase 1 requirements parsing failed/skipped. Architecture quality may be impacted."
            )
        elif context.refined_requirements.get("parsing_status") == "NeedsUserInput":
            return self._handle_error_and_halt(
                context, "Cannot proceed to Phase 2, Phase 1 requires user input."
            )
        ...
        try:
            ...
            if not architecture_llm_output or architecture_llm_output.error:
                logger.error(
                    "Halting phase due to initial architecture LLM call failure."
                )
                return context

            raw_architecture_text = architecture_llm_output.text
            logger.info(
                "Initial architecture call successful. Now calling LLM for parsing..."
            )
            logger.debug(
                f"Raw architecture text (start): {raw_architecture_text[:100]}..."
            )
            ...
            parsing_llm_output: Optional[LLMOutput] = self._call_llm(
                ...
                template_data={
                    "raw_llm_output": raw_architecture_text
                },  # Pass raw text
            )

            if not parsing_llm_output or parsing_llm_output.error:
                logger.error(
                    "LLM call for parsing architecture failed. Storing raw architecture text."
                )
                context.architecture_document = {
                    "raw_output": raw_architecture_text,
                ...
                return self._update_context_and_proceed(
                    context
                )  # Proceed with raw data? Or halt? Let's proceed.
            ...
            try:
                ...
                if isinstance(parsed_data, dict):
                    if parsed_data.get("error"):
                        logger.error(
                            f"Parsing LLM reported error: {parsed_data['error']}"
                        )
                        context.architecture_document = {
                            "raw_output": raw_architecture_text,
                            "parsing_status": f"Failed - Parser Error: {parsed_data['error']}",
                        }
                        ...
                        if isinstance(arch_doc, dict) and isinstance(tech_stack, list):
                            logger.info(
                                "Successfully parsed architecture JSON from parsing LLM."
                            )
                            context.architecture_document = arch_doc
                            context.technology_stack = tech_stack
                            context.architecture_document["parsing_status"] = (
                                "Success"  # Mark success
                            )
                        else:
                            logger.error(
                                "Parsing LLM JSON response missing required keys ('architecture_document' object, 'technology_stack' list) or keys have wrong type."
                            )
                            context.architecture_document = {
                                "raw_output": raw_architecture_text,
                                "parsing_status": "Failed - Invalid JSON structure from parser",
                                "parser_response": parsed_data,
                            }
            ...
            # Log summary
            arch_summary = str(context.architecture_document)[:200]
            logger.debug(f"Stored arch (summary): {arch_summary}...")
            logger.debug(f"Stored tech stack: {context.technology_stack}")

            # --- End Phase 2 Logic ---

            # 5. Update context status and return
            logger.info("Architecture generation & AI parsing attempt complete.")
