...
class LLMInput:
    # (Unchanged)
    prompt: str; model_identifier: str; temperature: float = 0.7; max_tokens: int = 3000; top_p: Optional[float] = None; relevant_context: Optional[Dict[str, Any]] = field(default_factory=dict)

@dataclass
class LLMOutput:
    # (Unchanged)
    text: str; model_identifier: Optional[str] = None; input_prompt: Optional[str] = None; raw_response: Optional[Any] = None; cost: Optional[float] = None; latency_ms: Optional[float] = None; error: Optional[str] = None; finish_reason: Optional[str] = None
    def to_dict(self): d = asdict(self); d.pop('input_prompt', None); d.pop('raw_response', None); return d

@dataclass
class ProjectContext:
    # (Fields unchanged - includes latest_user_response)
    project_id: str = field(default_factory=lambda: f"proj_{uuid.uuid4().hex[:8]}_{datetime.datetime.now().strftime('%Y%m%d')}")
    ...
    def add_log_entry(self, phase: str, level: str, message: str, error_details: Optional[str] = None):
        entry = {'timestamp': datetime.datetime.now(datetime.timezone.utc).isoformat(), 'phase': phase, 'level': level.upper(), 'message': message}
        if error_details: entry['details'] = error_details
        self.error_log.append(entry)
    def update_status(self, new_status: str, current_phase: Optional[str] = None):
        self.status = new_status;
        if current_phase: self.current_phase = current_phase
    def add_llm_call(self, llm_output: LLMOutput):
        call_record = llm_output.to_dict(); self.llm_call_history.append(call_record)

    # Method get_full_codebase (Unchanged)
    def get_full_codebase(self, max_len: int = 10000) -> str:
        full_code = ""; separator = "\n\n---\n\n"; logger.debug(f"Generating codebase string (max ~{max_len})..."); sorted_files = sorted(self.generated_code.keys())
        for filepath in sorted_files:
            code = self.generated_code[filepath]; header = f"--- File: {filepath} ---\n"; footer = "\n--- End File ---\n";
            if len(full_code) + len(header) + len(code) + len(footer) + len(separator) > max_len: logger.warning(f"Codebase truncated before {filepath}"); full_code += "\n[CODEBASE TRUNCATED]"; break
            full_code += separator + header + code + footer
        if not full_code and self.generated_code: full_code = "[CODEBASE TRUNCATED]"
        elif not self.generated_code: full_code = "[No code generated yet]"
        logger.debug(f"Codebase string length: {len(full_code)}"); return full_code.strip()
    ...
    def get_requirements_summary(self, format: str = 'yaml') -> str:
        ...
        if format == 'yaml':
            try:
                ...
                return yaml.dump(self.refined_requirements, default_flow_style=False, allow_unicode=True, sort_keys=False)
            except ImportError:
    ...
    def get_architecture_summary(self, format: str = 'yaml') -> str:
        ...
        if format == 'yaml':
            try:
                # PyYAML should be installed via requirements.txt
                return yaml.dump(self.architecture_document, default_flow_style=False, allow_unicode=True, sort_keys=False)
            except ImportError:
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'ProjectContext': instance = cls(**data); return instance
