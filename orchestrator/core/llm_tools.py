...
class LLMTool:
    ...
    RATE_LIMIT_WAIT_SECS = (
        25  # Default wait for 429 if Retry-After isn't provided (+ some buffer)
    )
    ...
    def __init__(self, api_key: str, model_identifier: str):
        ...
        if not api_key:
            logger.error(
                f"{self.__class__.__name__} needs API key for {model_identifier}. Non-functional."
            )
        self.api_key = api_key
        self.model_identifier = model_identifier
        logger.info(
            f"Initialized {self.__class__.__name__} for model {self.model_identifier}"
        )
    ...
    def execute(self, llm_input: LLMInput) -> LLMOutput:
        ...
        if not self.api_key:
            ...
            return LLMOutput(
                ...
                input_prompt=llm_input.prompt,
            )

        logger.debug(
            f"Executing LLM call with {self.__class__.__name__} for model {self.model_identifier}"
        )
        logger.debug(
            f"Input prompt (start): {llm_input.prompt[:100]}..."
        )  # Log less potentially sensitive data
        ...
        for attempt in range(
            self.MAX_RETRIES + 1
        ):  # +1 because range is exclusive, allows MAX_RETRIES retries
            ...
            try:
                ...
                if (
                    api_output.error
                ):  # Check if subclass reported a logical error despite successful call
                    output.error = api_output.error
                    logger.warning(
                        f"API call successful but tool reported error for {self.model_identifier}: {output.error}"
                    )
                    # Don't retry logical errors reported by the tool, break immediately
                    break
                else:
                    logger.info(
                        f"LLM call attempt {attempt + 1} successful for {self.model_identifier}."
                    )
                ...
                )  # Default to 500 if no response object

                # --- Handle Retryable Errors ---
                wait_time = 0
                if status_code == 429:  # Rate Limit
                    logger.warning(
                        f"Rate limit (429) hit for {self.model_identifier} on attempt {attempt + 1}/{self.MAX_RETRIES + 1}."
                    )
                    if is_last_attempt:
                        break  # Don't wait if it's the last attempt
                    # Check for Retry-After header
                    retry_after = e.response.headers.get("Retry-After")
                    if retry_after:
                        try:
                            wait_time = int(retry_after) + random.uniform(
                                0.5, 1.5
                            )  # Add jitter
                        except ValueError:
                            wait_time = self.RATE_LIMIT_WAIT_SECS * (
                                attempt + 1
                            )  # Fallback backoff
                    else:
                        # Exponential backoff + jitter for rate limit if no header
                        wait_time = min(
                            self.INITIAL_BACKOFF_SECS * (2**attempt),
                            self.MAX_BACKOFF_SECS,
                        ) + random.uniform(0, 1)
                        wait_time = max(
                            wait_time, self.RATE_LIMIT_WAIT_SECS
                        )  # Ensure minimum wait for rate limits

                elif 500 <= status_code < 600:  # Server Errors (5xx)
                    logger.warning(
                        f"Server error ({status_code}) for {self.model_identifier} on attempt {attempt + 1}/{self.MAX_RETRIES + 1}."
                    )
                    if is_last_attempt:
                        break
                    # Exponential backoff + jitter for server errors
                    wait_time = min(
                        self.INITIAL_BACKOFF_SECS * (2**attempt), self.MAX_BACKOFF_SECS
                    ) + random.uniform(0, 1)

                else:  # Non-retryable HTTP errors (e.g., 403 Forbidden, 401 Unauthorized, 400 Bad Request)
                    logger.error(
                        f"Non-retryable HTTP error ({status_code}) for {self.model_identifier}: {e}",
                        exc_info=True,
                    )
                    ...
                    if e.response is not None:
                        try:
                            resp_text = e.response.text[:200]  # Limit response snippet
                        except Exception:
                            pass
                ...
                logger.warning(
                    f"Timeout for {self.model_identifier} on attempt {attempt + 1}/{self.MAX_RETRIES + 1}."
                )
                if is_last_attempt:
                    break
                wait_time = min(
                ...
                last_error = e
                logger.error(
                    f"Connection/Request Error for {self.model_identifier} on attempt {attempt + 1}: {e}",
                    exc_info=True,
                )
                if is_last_attempt:
                    break
                ...
                err_msg = (
                    f"_execute_api_call not implemented for {self.__class__.__name__}"
                )
                logger.error(err_msg)
                output.error = err_msg
                last_error = NotImplementedError(err_msg)
                break  # Cannot retry this
                ...
                logger.critical(
                    f"Unexpected critical error in {self.__class__.__name__}._execute_api_call ({self.model_identifier}): {e}",
                    exc_info=True,
                )
                output.error = err_msg
        ...
        if (
            attempt == self.MAX_RETRIES and output.error is None
        ):  # Check if we exited loop due to retries but error wasn't set by non-retryable path
            if last_error:
                error_context = (
        ...
        logger.log(
            log_level,
            f"LLM execution finished: {self.model_identifier}. Latency: {output.latency_ms:.2f} ms. Status: {'FAILED' if output.error else 'Success'}. Error: {output.error or 'None'}",
        )
...
class OpenAITool(LLMTool):
    ...
    def __init__(
        self, api_key: str = config.OPENAI_API_KEY, model_identifier: str = "gpt-4"
    ):
        super().__init__(api_key, model_identifier)
    ...
    def _execute_api_call(self, llm_input: LLMInput) -> LLMOutput:
        ...
        payload = {
            ...
            "messages": [{"role": "user", "content": llm_input.prompt}],
        ...
        response = requests.post(
            self.API_ENDPOINT, json=payload, headers=headers, timeout=180
        )
        ...
        if data.get("choices") and data["choices"]:
            ...
            output_text = choice.get("message", {}).get("content", "").strip()
        if not output_text:
            logger.warning(
                f"OpenAI response empty. Finish:{finish_reason}. M:{self.model_identifier}"
            )
        # TODO: cost = calculate_openai_cost(...)
        return LLMOutput(
            text=output_text, raw_response=data, cost=cost, finish_reason=finish_reason
        )
...
class AnthropicTool(LLMTool):
    ...
    def __init__(
        ...
        model_identifier: str = "claude-3-opus-20240229",
    ):
        super().__init__(api_key, model_identifier)
    ...
    def _execute_api_call(self, llm_input: LLMInput) -> LLMOutput:
        ...
        payload = {
            "model": self.model_identifier,
            "system": llm_input.relevant_context.get(
        ...
        if not output_text:
            logger.warning(
                f"Anthropic response empty. Finish:{finish_reason}. M:{self.model_identifier}"
            )
...
class LLMFactory:
    ...
    def get_tool(model_identifier: Optional[str]) -> LLMTool:
        ...
        if not model_identifier:
            ...
            logger.warning(f"No model specified, using default: {default_model}")
            model_identifier = default_model  # Changed default here too
        model_id_lower = model_identifier.lower()
        provider_prefix = None
        if ":" in model_id_lower:
            try:
                provider_prefix, model_id_lower = model_id_lower.split(":", 1)
            except ValueError:
                logger.warning(f"Invalid model format: {model_identifier}")
        if model_identifier in LLMFactory._tool_cache:
            cached_tool = LLMFactory._tool_cache[model_identifier]
            key_attr = None
            if isinstance(cached_tool, OpenAITool):
                key_attr = config.OPENAI_API_KEY
            elif isinstance(cached_tool, AnthropicTool):
                key_attr = config.ANTHROPIC_API_KEY
            # Add checks for other tools
            if key_attr:
                cached_tool.api_key = (
                    key_attr  # Update key in case config changed? Risky maybe.
                )
            else:
                logger.warning(
                    f"API Key missing in config for cached {model_identifier}. Re-creating."
                )
            if cached_tool.api_key:
                return cached_tool
            else:
                logger.warning(f"Cached {model_identifier} lacks key, re-creating.")
        logger.info(f"Creating LLMTool instance for: {model_identifier}")
        tool_instance = None
        error_message = None
        try:
            if provider_prefix == "openai" or "gpt" in model_id_lower:
                if not config.OPENAI_API_KEY:
                    ...
                    tool_instance = OpenAITool(
                        api_key=config.OPENAI_API_KEY, model_identifier=model_identifier
                    )
            elif provider_prefix == "anthropic" or "claude" in model_id_lower:
                if not config.ANTHROPIC_API_KEY:
                    error_message = "Anthropic API key missing."
                else:
                    tool_instance = AnthropicTool(
                        api_key=config.ANTHROPIC_API_KEY,
                        model_identifier=model_identifier,
                    )
            # Add elif for other providers
            else:
                error_message = f"Unsupported model identifier: {model_identifier}"
                logger.error(error_message)
        except Exception as e:
            logger.error(
                f"Error instantiating LLM tool for {model_identifier}: {e}",
                exc_info=True,
            )
            raise ValueError(
                f"Failed to create instance for {model_identifier}: {e}"
            ) from e
        if error_message:
            raise ValueError(
                f"Cannot create tool for '{model_identifier}': {error_message}"
            )
        if tool_instance and tool_instance.api_key:
            LLMFactory._tool_cache[model_identifier] = tool_instance
            logger.info(f"Created/cached LLMTool for {model_identifier}")
            return tool_instance
        elif tool_instance:
            raise ValueError(f"Tool '{model_identifier}' missing API key.")
        else:
            raise ValueError(f"Failed to create instance for: {model_identifier}")
