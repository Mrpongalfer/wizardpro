...
_log_level_env = os.getenv("LOG_LEVEL", _default_log_level_str).upper() # Check env var first
...
logging.basicConfig(level=_numeric_log_level, format='%(asctime)s - %(name)s - %(levelname)s - [%(module)s.%(funcName)s:%(lineno)d] - %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
logger = logging.getLogger(__name__) # Get logger for this module


# Load environment variables from .env file
if os.path.exists(_dotenv_path):
    loaded = load_dotenv(dotenv_path=_dotenv_path, override=True) # override=True ensures .env takes precedence over system env vars
    if loaded:
        logger.info(f"Successfully loaded environment variables from: {_dotenv_path}")
    else:
        logger.warning(f".env file found at {_dotenv_path}, but loading might have failed (check permissions?).")
else:
    logger.warning(f".env file not found at {_dotenv_path}. Relying solely on system environment variables.")
...
if _db_vars_exist:  # Check if all required DB vars are present
    ...
    missing_vars = [var for var in ['DB_HOST', 'DB_PORT', 'DB_NAME', 'DB_USER', 'DB_PASSWORD'] if not locals().get(var)]
    if missing_vars: # Only warn if some vars were expected but missing
         logger.warning(f"Database configuration incomplete in environment. Missing: {', '.join(missing_vars)}. DATABASE_CONNECTION_INFO set to None.")
...
if LOG_LEVEL_STR not in valid_log_levels:
    logger.warning(f"Invalid LOG_LEVEL '{LOG_LEVEL_STR}' found in environment. Using {_default_log_level_str}.")
...
if not OPENAI_API_KEY and not ANTHROPIC_API_KEY and not QWEN_API_KEY:
    logger.warning("No common LLM API key found in environment variables (OpenAI, Anthropic, Qwen checked).")

if not DATABASE_CONNECTION_INFO and _db_vars_exist: # Warn only if vars were present but construction failed
     logger.error("Database connection variables seem present, but constructing connection info failed. Check DB_TYPE and variable formats.")

if not os.path.isdir(PROMPT_DIR_PATH):
     logger.error(f"Prompt directory specified does not exist or is not a directory: {PROMPT_DIR_PATH}. Please create it or correct the PROMPT_DIR environment variable.")
else:
